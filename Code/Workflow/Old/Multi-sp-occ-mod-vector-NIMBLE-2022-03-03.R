...
#  
 ...
 # object = bee.plant.obs
 # 2-D matrix
  bee.plant.obs <- read.csv( "./Data/bee-plant-obs-long-format 2022 04 05 - short plant list - no apis.csv")
 ...
 # Load covariates
 load("~/Dropbox/Globi/Data/model_covariates - 2022 04 05 - no apis.rds")
  ...
  MEcode <- nimbleCode({
    ...
    for(i in 1:n.row.true){
      ...
      logit(psi[bee.ID.true[i], plant.ID.true[i], month.ID.true[i]]) <- u[bee.ID.true[i]] +
                                                                        # Bee size
                                                                        beta_psi_B_size * size[bee.ID.true[i]] + 
                                                                        # Bee sociality
                                                                        beta_psi_B_sociality * sociality[bee.ID.true[i]] +
                                                                        # Flower color
                                                                        beta_psi_F_color * flower_color[plant.ID.true[i]] +
                                                                        # Flower shape
                                                                        beta_psi_F_shape * flower_shape[plant.ID.true[i]]
    ...
    for(i in 1:n.row.obs){
      ...
      logit(p[bee.ID.obs[i], plant.ID.obs[i], month.ID.obs[i], citation.ID.obs[i]]) <- v[bee.ID.obs[i]] +
                                                                                        # Bee Strippiness
                                                                                        beta_p_B_stripped * stripped[bee.ID.obs[i]] +
                                                                                        # Bee size
                                                                                        beta_p_B_size * size[bee.ID.obs[i]] +
                                                                                        # Quadratic term for month
                                                                                        beta_p_month_1 * month.ID.obs[i] +
                                                                                        beta_p_month_2 * pow(month.ID.obs[i], 2) +
                                                                                        # source
                                                                                        beta_p_source * citation.code[citation.ID.obs[i]]+
                                                                                        # Flower color
                                                                                        beta_p_F_color * flower_color[plant.ID.obs[i]] +
                                                                                        # Flower shape
                                                                                        beta_p_F_shape * flower_shape[plant.ID.obs[i]]
...
save(out, 
     file = "./ModelOutput/globi-short plant list- 2022 04 05 - all cov - NO apis - NIMBLE.rds")

save(result, 
     file = "./ModelOutput/OUTPUT - globi-short plant list- 2022 04 05 - all cov - NO apis - NIMBLE.rds")

save(MCMClist,
     file = "./ModelOutput/MCMClist- globi-short plant list- 2022 04 05 - all cov - NO apis - NIMBLE.rds")
