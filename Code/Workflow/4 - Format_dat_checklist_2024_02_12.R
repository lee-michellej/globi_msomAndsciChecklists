...
# How many rows will be kept?
  # 5,158 --> 6 april 2024, 9555 rows will be kept
...
for(i in 1:nrow(dat2)){
  ...
  first.col <- which(dat2$sourceInstitutionCode[i] == institution.codes$fullcode)
  sec.col <- which(dat2$sourceInstitutionCode[i] == institution.codes$firstcode)
  third.col <- which(dat2$sourceInstitutionCode[i] == institution.codes$secondcode)
...
# eventDateUnixEpoch
#dat2$eventDate <- as.POSIXct(dat2$eventDateUnixEpoch/1000, origin = "1970-01-01")
    ...
    for(k in 1:dim(bee.plant.date.cite)[3]){ # for each month
      ...
      # There is a row in the plant.phenology table that is all NA - we need to kip it
        ...
        if(# If the bee is active (bee.phenology == 1)
          ...
          # start with January (column == 27)
          plant.phenology2[j, k + 26] == 1){
...
# How many possible interactions are there?
  # 216,385
  # although previous length of bee.plant.inter is 57374
...
# Save the bee.plant.inter file
 ...
 save(bee.plant.inter, file= "./Data/bee_plant_inter_2024_04_07 - short plant - no apis.rds")
...
write.csv(obs.not.possible.globi,
          file = "./Data/globi-obs-not-possible 2024 04 08 - no apis.csv")
