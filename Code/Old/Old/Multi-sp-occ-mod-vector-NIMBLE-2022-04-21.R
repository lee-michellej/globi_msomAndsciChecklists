  ...
  MEcode <- nimbleCode({
    ...
    for(i in 1:n.row.obs){
      ...
      logit(p[bee.ID.obs[i], plant.ID.obs[i], month.ID.obs[i], citation.ID.obs[i]]) <- v[bee.ID.obs[i]] +
                                                                                        ...
                                                                                        # Plant family
                                                                                        beta_p_F_family * plant_family[plant.ID.obs[i]]
...
save(out, 
     file = "./ModelOutput/globi-short plant list- 2022 04 21 - all cov - NO apis - NIMBLE.rds")

save(result, 
     file = "./ModelOutput/OUTPUT - globi-short plant list- 2022 04 21 - all cov - NO apis - NIMBLE.rds")

save(MCMClist,
     file = "./ModelOutput/MCMClist- globi-short plant list- 2022 04 21 - all cov - NO apis - NIMBLE.rds")
