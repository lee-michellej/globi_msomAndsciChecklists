...
# Confirm that Apis mellifera DOESNOT appear
...
# Total number of unique species names in the synonym list: 
  # 20,460
length(unique(bee.names$resolvedName))

# Do all of the bee species in our checklist appear in the synoynm list?
  # There is 1 FALSE
which(bee.list$scientificName %in% bee.names$providedName == FALSE)
...
# Use a loop to go through each globi bee species list and try to find a match in the synonym list - if there is a match, then assign the resolvedName
  # This loop takes ~ 8 minutes to run but it depends on your computer
start.time <- Sys.time()
...
# Determine if there is a match between Globi and the bee and plant checklists
  # This loop takes: ~7 minutes to run but it depends on your computer
start.time <- Sys.time()
for(i in 1:nrow(dat1)){
...
ggsave("./Figures/2023_02_24/Fig. S1 - Globi_CA_map.pdf", 
       height = 10, width = 12)
...
# But we also need to look through 1 other column because sometimes the sourceInstitutionCode doesn't contain all of the information about the instuition:
  # sourceCatalogNumber
...
# We get a warning for 5 rows: 359, 399, 402, 404, 405
dat6[c(359, 399, 402, 404, 405),]
  # These rows have a code 
dat6[c(359, 399, 402, 404, 405),]$code
  # But no num
dat6[c(359, 399, 402, 404, 405),]$num

# View(data.frame(dat6$code, dat6$num))


# Now we have a code column from dat6 we should use to match with the institution code file
...
for(i in 1:nrow(dat6)){
  ...
  if(length(first.col) > 0){
    ...
    dat6$resolvedSource[i] <- as.character(institution.codes$institution[sec.col])[1]
...
# 11. Determine which bee-plant interactions are possible & which are forbidden -------------------------------------------------------
 ...
 # Save the bee.plant.inter file
  ...
  save(bee.plant.inter, file= "./Data/bee_plant_inter_2023_02_24 - short plant - no apis.rds")
...
# How many detections are in the dataframe?
  # 211
  # Note that in some cases the same citation documents the same bee and plant interactions during the same month, that is why this value is < nrow(dat7) - which = 358
length(which(bee.plant.date.cite == 1))


# Confirm that there are duplicate interactions in the dat7 dataframe
  # Count the number of unique combinations of bee species, plant, source, and month
  # This should match the number of 1's in the bee.plant.date.cite array
length(which(duplicated(data.frame(dat7$resolvedBeeNames, dat7$resolvedPlantNames, dat7$resolvedSource, dat7$month)) == FALSE))
  # Perfect!
...
# Convert the 4-D array into 2-D (long format)
bee.plant.obs2 <- melt(bee.plant.date.cite)

# Add column names
colnames(bee.plant.obs2) <- c("bee.name", "plant.name", "month", "source", "y")

head(bee.plant.obs2)

# Remove NA rows
bee.plant.obs3 <- bee.plant.obs2[is.na(bee.plant.obs2$y) == FALSE,]

# Number of rows
nrow(bee.plant.obs3)


# Add some columns to add the ID numbers
bee.plant.obs3$beeID <- NA
bee.plant.obs3$plantID <- NA
bee.plant.obs3$monthID <- NA
bee.plant.obs3$sourceID <- NA



# Fill in ID columns with values
for(i in 1:nrow(bee.plant.obs3)){
  
  # Determine which citation
  cit.ID <- which(citations %in% bee.plant.obs3$source[i] == TRUE)
  
  # Determine which bee
  bee.ID <- which(bee.species %in% bee.plant.obs3$bee.name[i]  == TRUE)
  
  # Determine which month
  month.ID <- as.numeric(bee.plant.obs3$month[i])
  
  # Determine which plant
  plant.ID <- which(plant.species %in% bee.plant.obs3$plant.name[i]  == TRUE)
  
  # Add values
  bee.plant.obs3$beeID[i] <- bee.ID
  bee.plant.obs3$plantID[i] <- plant.ID
  bee.plant.obs3$monthID[i] <- month.ID
  bee.plant.obs3$sourceID[i] <- cit.ID
...
# How many observed bee-plant-month-citation observations were there?
length(which(bee.plant.obs3$y > 0))


# Rename
bee.plant.obs <- bee.plant.obs3
...
# Save the bee.plant.obs file
  # This file takes a while to generate - so in the future - we can just import it into this space
 save(bee.plant.obs, file= "./Data/bee_plant_obs_2023_02_23 - short plant list - no apis.rds")
...
# Observation matrix = (all bees x all plants x ONLY months when sources went out x all citations)
  # bee.plant.obs
head(bee.plant.obs)
nrow(bee.plant.obs)
  # 142,146

# ALL possible bee-plant-month combinations (all bees x all plants x all months)
  # bee.plant.inter
...
nrow(bee.plant.inter)
 ...
 length(which(bee.plant.obs$y > 0))
 length(which(bee.plant.obs$y == 0))
...
# Convert the 3-D array into 2-D (long format)
bee.plant.date.long <- melt(bee.plant.date)

# Add column names
colnames(bee.plant.date.long) <- c("bee.name", "plant.name", "month", "y")

head(bee.plant.date.long)

# Remove NA rows
bee.plant.date.long <- bee.plant.date.long[is.na(bee.plant.date.long$y) == FALSE,]

# Number of rows
nrow(bee.plant.date.long)


# Add some columns to add the ID numbers
bee.plant.date.long$beeID <- NA
bee.plant.date.long$plantID <- NA
bee.plant.date.long$monthID <- NA



# Fill in ID columns with values
for(i in 1:nrow(bee.plant.date.long)){
  

  # Determine which bee
  bee.ID <- which(bee.species %in% bee.plant.date.long$bee.name[i]  == TRUE)
  
  # Determine which month
  month.ID <- as.numeric(bee.plant.date.long$month[i])
  
  # Determine which plant
  plant.ID <- which(plant.species %in% bee.plant.date.long$plant.name[i]  == TRUE)
  
  # Add values
  bee.plant.date.long$beeID[i] <- bee.ID
  bee.plant.date.long$plantID[i] <- plant.ID
  bee.plant.date.long$monthID[i] <- month.ID
...
nrow(observed.but.not.possible) + nrow(bee.plant.inter)
...
# Now, we have to determine which row in the observed.but.not.possible object is associated with each interaction in the Globi database
obs.not.possible.globi <- data.frame()

for(i in 1:nrow(observed.but.not.possible)){

  # Determine which rows match with the criteria
  globi.row <- which( dat7$resolvedBeeNames == observed.but.not.possible$bee.name[i]  &
                      dat7$resolvedPlantNames == observed.but.not.possible$plant.name[i] & 
...
#           file = "./Data/globi-obs-not-possible 2023 02 24 - no apis.csv")
...
# In excel, I assigned a new month to this "not possible interactions" using the phenology data
  # Shifted month observations by 1 - 4 months to fall within active window
...
# Copy original 4-D arry to save a copy
bee.plant.date.cite.ORG <- bee.plant.date.cite

# Read in new file with a new.month column
new.month <- read.csv("./Data/globi-obs-not-possible 2023 02 24 - no apis - new month.csv")
...
for(i in 1:nrow(observed.but.not.possible)){
  ...
  citID <- which(bee.plant.date.cite[observed.but.not.possible$beeID[i], 
                                     ...
                                     observed.but.not.possible$monthID[i], ] == 1)
...
211 - nrow(observed.but.not.possible)
...
# Next, check to make sure that everything matches between the observations and the interactions that are possible
  # This code is similar to what we ran above
bee.plant.date <- apply(bee.plant.date.cite, c(1, 2, 3), max, na.rm = TRUE) 
bee.plant.date[bee.plant.date == "-Inf"] <- NA
...
new.bee.plant.month.dat <- data.frame(new.month$BeeID,
                                      new.month$PlantID,
                                      new.month$SourceID)

nrow(new.bee.plant.month.dat)

# We are going from 45 entries to 24 because the new months make some combinations duplicates now
nrow(new.bee.plant.month.dat[which(duplicated(new.bee.plant.month.dat) == FALSE),])
...
length(which(bee.plant.date.long$y > 0))
...
save(bee.plant.date.cite, 
     file= "./Data/data_summary/globi_data_formatted_bee_plant_date_citation_2023_02_23 - short plant list - no apis.rds")


## Save the subsetted Globi data
save(dat7, 
     file= "./Data/data_summary/FINAL - subsetted_globi_data_2023 02 23.csv" )
